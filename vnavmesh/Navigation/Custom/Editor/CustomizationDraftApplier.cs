...
{
    public static void ApplyScene(SceneExtractor scene, CustomizationDraft draft)
    {
    ...
    public static void ApplyBuildProfile(NavmeshBuildProfile profile, CustomizationDraft draft)
    {
    ...
    public static void ApplyBuildSettings(NavmeshSettings settings, CustomizationDraft draft)
    {
    ...
    public static void ApplySettings(DtNavMeshCreateParams config, CustomizationDraft draft)
    {
    ...
    private static void ApplyColliderInsertion(SceneExtractor scene, DraftSceneColliderInsertion insertion)
    {
        if (!insertion.Enabled)
            return;

        var (min, max) = NormalizeBounds(insertion.Min, insertion.Max);
        var bounds = new FFXIVClientStructs.FFXIV.Common.Component.BGCollision.Math.AABB
        {
            Min = min,
            Max = max
        };
    ...
    private static void ApplyInstancePatch(SceneExtractor scene, DraftSceneInstancePatch patch)
    {
    ...
    private static void ApplyPartPatch(SceneExtractor scene, DraftScenePartPatch patch)
    {
        if (!patch.Enabled || string.IsNullOrWhiteSpace(patch.MeshKey) || !scene.Meshes.TryGetValue(patch.MeshKey, out var mesh))
            return;
    ...
    private static int ResolveInstanceIndex(SceneExtractor.Mesh mesh, DraftSceneInstancePatch patch)
    {
    ...
    private static (Vector3 Min, Vector3 Max) NormalizeBounds(Vector3 a, Vector3 b) =>
        (Vector3.Min(a, b), Vector3.Max(a, b));

    private static FFXIVClientStructs.FFXIV.Common.Component.BGCollision.Math.AABB CalculateLocalBounds(List<SceneExtractor.MeshPart> parts)
    {
        var bounds = new FFXIVClientStructs.FFXIV.Common.Component.BGCollision.Math.AABB
        {
    ...
    private static FFXIVClientStructs.FFXIV.Common.Component.BGCollision.Math.AABB CalculateLocalBounds(List<Vector3> vertices)
    {
        var bounds = new FFXIVClientStructs.FFXIV.Common.Component.BGCollision.Math.AABB
        {
            Min = new(float.MaxValue),
