...
)
{
    ...
    private static PostprocessedPath BuildPath(PlannerResult result, IReadOnlyList<PostprocessedPathSegment> segments) =>
        new()
        {
    ...
    private List<Vector3> BuildStraightPathWaypoints(PlannerPathSegment segment, long[] corridor)
    {
        ...
        return DeduplicateWaypoints(straightPath.AsSpan(0, straightPathCount).ToArray().Select(point => point.pos.RecastToSystem()));
    }
    ...
    private List<Vector3> BuildRawStraightPathWaypoints(PlannerPathSegment segment, long[] corridor, int straightPathOptions)
    {
        ...
        return [.. straightPath.AsSpan(0, straightPathCount).ToArray().Select(point => point.pos.RecastToSystem())];
    }

    private GroundCorridorPayload? BuildGroundCorridor(PlannerPathSegment segment, CancellationToken cancel) =>
        BuildGroundCorridor(segment, 0, true, cancel);

    private GroundCorridorPayload? BuildGroundCorridor(PlannerPathSegment segment, int straightPathOptions, CancellationToken cancel) =>
    ...
    private static int ResolveInitialSourceIndex(PlannerPathSegment segment, DtStraightPath[] straightPath, int straightPathCount)
    {
    ...
    private static int ResolveInitialCornerIndex(IReadOnlyList<GroundPathCorner> corners, int initialSourceIndex)
    {
        if (corners.Count == 0)
            return 0;
    ...
    private static List<Vector3> BuildRawDiscreteWaypoints(PlannerPathSegment segment) => [.. segment.Points];
