...
)
{
    ...
    private bool CanUseShortcut(IReadOnlyList<GroundPathCorner> corners, int startIndex, int endIndex)
    {
    ...
    private bool HasLineOfSight(GroundPathCorner start, GroundPathCorner end)
    {
    ...
    private bool TryRaycastFromPoly(long startRef, Vector3 start, Vector3 end)
    {
        ...
        if (!meshQuery.GetPolyHeight(startRef, startProbe.SystemToRecast(), out var startHeight).Succeeded())
            return false;
        ...
        if (!meshQuery.GetPolyHeight(endRef, endProbe.SystemToRecast(), out var height).Succeeded())
            return false;

        return MathF.Abs(height - end.Y) <= MAX_SHORTCUT_VERTICAL_ERROR;
    }

    private bool TryResolveStartPoly(Vector3 position, long preferredRef, out long polyRef, out Vector3 projectedPosition)
    {
        if (preferredRef != 0 && meshQuery.GetPolyHeight(preferredRef, position.SystemToRecast(), out var preferredHeight).Succeeded())
        {
        ...
        projectedPosition = nearest.RecastToSystem();
        return status.Succeeded() && polyRef != 0 && isOverPoly;
    }

    private bool TryProjectToMesh(Vector3 candidate, long preferredRef, out Vector3 projected, out long polyRef)
    {
    ...
    private static bool CanSkipRange(IReadOnlyList<GroundPathCorner> corners, int start, int end, int initialSourceIndex)
    {
    ...
    private static bool IsProtected(IReadOnlyList<GroundPathCorner> corners, int index, int initialSourceIndex) =>
        corners[index].SourceIndex < initialSourceIndex ||
        IsSemanticAnchor(corners[index])                ||
        index > 0 && IsSemanticAnchor(corners[index - 1]);

    private static bool IsSemanticAnchor(GroundPathCorner corner) =>
        corner.LinkKind != null ||
        (corner.StraightPathFlags & DtStraightPathFlags.DT_STRAIGHTPATH_OFFMESH_CONNECTION) != 0;

    private static Vector2 HorizontalDelta(Vector3 from, Vector3 to) => new(to.X - from.X, to.Z - from.Z);

    private static float HorizontalDistance(Vector3 left, Vector3 right) => HorizontalDelta(left, right).Length();

    private static float DirectionDot(Vector3 previous, Vector3 current, Vector3 next)
    {
    ...
    private static void AppendDistinct(List<GroundPathCorner> result, GroundPathCorner corner)
    {
        if (result.Count == 0 ||
