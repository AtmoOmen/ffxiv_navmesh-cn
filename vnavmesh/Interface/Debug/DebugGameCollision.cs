...
public unsafe class DebugGameCollision : IDisposable
{
    ...
    private void VisualizeColliderMeshPCBNode
        (MeshPCB.FileNode* node, ref Matrix4x3 world, Vector4 color, ulong objMatId, ulong objMatInvMask, BitMask filterId, BitMask filterMask)
    {
        if (node == null)
            return;
        ...
        if (node->NumPrims > 0)
        {
            ...
            foreach (ref var prim in node->Primitives)
            {
                ...
                if (pass) renderer.AddTriangle(prim.V1, prim.V3, prim.V2); // change winding to what dx expects
                else renderer.AddTriangle(prim.V1,      prim.V1, prim.V1); // TODO: avoid degenerates...
