...
public unsafe class DebugLayout : IDisposable
{
		...
		if (ni.Opened)
		{
			...
			switch (inst->Id.Type)
			{
				...
				case InstanceType.CollisionBox:
					...
					var pcbPath = instCollGeneric->PcbPathCrc != 0 ? LayoutUtil.FindPtr(ref layout->CrcToPath, instCollGeneric->PcbPathCrc) : null;
	...
	{
		using var n = _tree.Node($"{tag}: {header->Id} '{LayoutUtil.ReadString(header->LayerGroupName)}', {header->NumLayers} layers at +{header->OffsetLayers}", header->NumLayers == 0);
		if (!n.Opened)
			return false;
		foreach (var layerOffset in header->LayerOffsets)
		{
			...
			if (nl.Opened)
			{
				foreach (var instOffset in layer->InstanceOffsets)
				{
					...
					using var ni = _tree.Node($"[{instance->Key:X}] '{LayoutUtil.ReadString(instance->Name)}': type={instance->Type}{instUnk}, trans={instance->Transform.Translation}, rot={instance->Transform.Rotation}, scale={instance->Transform.Scale}, offset=+{layer->OffsetInstances}+{instOffset}");
					if (ni.Opened)
					{
	...
	private void FillInstancesFromFileLayerGroup(FileLayerGroupHeader* lg, uint filterId, Span<GameMain.Festival> festivals)
	{
		...
		foreach (var layerOff in lg->LayerOffsets)
		{
			...
			bool expectedInGame = LayoutUtil.LayerActiveFestival(layer, festivals) && LayoutUtil.LayerActiveFilter(layer, filterId);
