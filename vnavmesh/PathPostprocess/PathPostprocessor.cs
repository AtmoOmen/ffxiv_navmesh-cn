...
internal sealed class PathPostprocessor
{
    ...
    public PostprocessedPath CreateExternalInputPath(List<Vector3> waypoints, MovementMode requestedMode, Vector3 requestedDestination, float destinationTolerance, float completionTolerance)
    {
    ...
    private IReadOnlyList<Vector3> BuildMeshWaypoints(PlannerPathSegment segment, bool useStringPulling)
    {
        ...
        if (useStringPulling)
        {
            ...
            var straightStatus = _meshQuery.FindStraightPath(segment.StartPosition.SystemToRecast(), segment.EndPosition.SystemToRecast(), [.. segment.Corridor], ref straightPath, 1024, 0);
