...
public class NavmeshQuery
{
    ...
    internal PlannerResult PlanMeshPathDetailed(Vector3 from, Vector3 to, bool useRaycast, float range, CancellationToken cancel)
    {
        ...
        if (requestedStartRef == 0 || endRef == 0) return LogMeshFailure(from, to, requestedStartRef, endRef, 0, range, "无法在导航网格上找到起点或终点多边形");
        ...
        if (bestStartCandidate == null) return LogMeshFailure(from, to, requestedStartRef, endRef, 0, range, "无法为起点选择可用的导航多边形");
    ...
    internal PlannerResult PlanVolumePathDetailed(Vector3 from, Vector3 to, bool useRaycast, CancellationToken cancel)
    {
        ...
        var voxelPath = VolumeQuery.FindPath
            (startVoxel, endVoxel, from, to, useRaycast, false, cancel); // TODO: do we need intermediate points for string-pulling algo?
