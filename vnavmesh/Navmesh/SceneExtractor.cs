...
public class SceneExtractor
{
        ...
        if (analytic)
        {
            if (scene.AnalyticShapes.TryGetValue(crc, out var shape))
            {
                ...
                var (path, bounds) = (FileLayerGroupAnalyticCollider.Type)shape.transform.Type switch
                {
                    ...
                    FileLayerGroupAnalyticCollider.Type.Cylinder => (_keyMeshCylinder,
                                                                        CalculateBoxBounds
        ...
        (SceneDefinition scene, ulong key, Transform instanceTransform, uint crc, ColliderType type)
    {
        ...
        {
            ColliderType.Box           => (_keyAnalyticBox, CalculateBoxBounds(ref transform)),
            ColliderType.Sphere        => (_keyAnalyticSphere, CalculateSphereBounds(key, ref transform)),
            ColliderType.Cylinder      => (_keyAnalyticCylinder, CalculateBoxBounds(ref transform)),
            ColliderType.Plane         => (_keyAnalyticPlaneSingle, CalculatePlaneBounds(ref transform)),
            ColliderType.Mesh          => (scene.MeshPaths[crc], CalculateMeshBounds(Meshes[scene.MeshPaths[crc]], ref transform)),
            ColliderType.PlaneTwoSided => (_keyAnalyticPlaneDouble, CalculatePlaneBounds(ref transform)),
            _                          => ("", default)
        };
        return (path, transform, bounds);
    }
