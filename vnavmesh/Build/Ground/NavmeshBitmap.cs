...
public sealed class NavmeshBitmap
{
    ...
    {
        [FieldOffset(0)]
        public ushort Type; // 0x4D42 'BM'

        [FieldOffset(2)]
        public int Size; // size of the file in bytes

        [FieldOffset(6)]
        public uint Reserved;

        [FieldOffset(10)]
        public int OffBits; // offset from this to pixel data
    }
    ...
    public int CoordToIndex
    (
        int x,
        int y
    ) => (y * BytesPerRow) + (x >> 3);

    public byte CoordToMask
    (
        int x
    ) => (byte)(0x80u >> (x & 7));

    public ref byte ByteAt
    (
        int x,
        int y
    ) => ref Pixels[CoordToIndex(x, y)];
    ...
    )
    {
        ...
        (
            ...
            {
                SizeInBytes     = Marshal.SizeOf<BitmapInfoHeader>(), Width = Width, Height = -Height, PlaneCount = 1, BitCount = 1, XPixelsPerMeter = intRes,
                YPixelsPerMeter = intRes
            }
        );
    ...
    )
    {
        ...
        for (var i = 1; i < poly.vertCount; ++i)
            edges[i] = verts[i - 1] - verts[i];

        var     x0 = Math.Clamp((int)MathF.Floor((min.X   - MinBounds.X) * Resolution), 0, Width - 1);
        var     z0 = Math.Clamp((int)MathF.Floor((min.Z   - MinBounds.Z) * Resolution), 0, Height - 1);
        var     x1 = Math.Clamp((int)MathF.Ceiling((max.X - MinBounds.X) * Resolution), 0, Width - 1);
        var     z1 = Math.Clamp((int)MathF.Ceiling((max.Z - MinBounds.Z) * Resolution), 0, Height - 1);
    ...
    (
        DtMeshTile tile,
        int        i
    ) => new(tile.data.verts[i * 3], tile.data.verts[(i * 3) + 1], tile.data.verts[(i * 3) + 2]);
    ...
    public static unsafe T ReadStruct<T>
    (
        Stream stream
    ) where T : unmanaged
    {
    ...
    public static unsafe void WriteStruct<T>
    (
        Stream stream,
        in T   value
    ) where T : unmanaged
