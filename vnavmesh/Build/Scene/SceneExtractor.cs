...
public class SceneExtractor
{
    ...
    public (string path, Matrix4x3 transform, AABB bounds) ExtractBgPartInfo
    (
        SceneDefinition scene,
        ulong           key,
        ...
        if (analytic)
        {
            if (scene.AnalyticShapes.TryGetValue(crc, out var shape))
            {
                ...
                var (path, bounds) = (FileLayerGroupAnalyticCollider.Type)shape.transform.Type switch
                {
                    ...
                    FileLayerGroupAnalyticCollider.Type.Sphere => (KEY_ANALYTIC_SPHERE, CalculateSphereBounds(key, ref resultingTransform)),
                    FileLayerGroupAnalyticCollider.Type.Cylinder => (KEY_MESH_CYLINDER,
                                                                        CalculateBoxBounds
                                                                            (ref resultingTransform)), // TODO: we can probably do a tighter fit for cylinders...
                    FileLayerGroupAnalyticCollider.Type.Plane => (KEY_ANALYTIC_PLANE_SINGLE, CalculatePlaneBounds(ref resultingTransform)),
                    _                                         => ("", default)
                };
    ...
    public (string path, Matrix4x3 transform, AABB bounds) ExtractColliderInfo
    (
        SceneDefinition scene,
        ulong           key,
        Transform       instanceTransform,
    ...
    )
    {
        ...
        var (path, bounds) = type switch
        {
            ...
            ColliderType.Sphere        => (KEY_ANALYTIC_SPHERE, CalculateSphereBounds(key, ref transform)),
            ColliderType.Cylinder      => (KEY_ANALYTIC_CYLINDER, CalculateBoxBounds(ref transform)),
            ColliderType.Plane         => (KEY_ANALYTIC_PLANE_SINGLE, CalculatePlaneBounds(ref transform)),
            ColliderType.Mesh          => (scene.MeshPaths[crc], CalculateMeshBounds(Meshes[scene.MeshPaths[crc]], ref transform)),
            ColliderType.PlaneTwoSided => (KEY_ANALYTIC_PLANE_DOUBLE, CalculatePlaneBounds(ref transform)),
            _                          => ("", default)
        };
        return (path, transform, bounds);
    }

    private static Mesh CreateBuiltinMesh
    (
        List<MeshPart> parts,
        MeshType       type
    ) => new()
    ...
    )
    {
        ...
        if (f != null)
        {
            fixed (byte* rawData = &f.Data[0])
            {
                ...
                if (data->Version is 1 or 4) FillFromFileNode(mesh.Parts, (MeshPCB.FileNode*)(data + 1));
            }
        }
    ...
    (
        Mesh          mesh,
        ulong         id,
        ref Matrix4x3 worldTransform,
        ref AABB      worldBounds,
        ulong         matId
    )
    {
    ...
    private static AABB CalculateSphereBounds
    (
        ulong         id,
        ref Matrix4x3 world
    )
    ...
    private static AABB CalculateMeshBounds
    (
        Mesh          mesh,
        ref Matrix4x3 world
    )
    ...
    private static AABB CalculateTransformedBounds
    (
        AABB          bounds,
        ref Matrix4x3 world
    )
    ...
    private static AABB CalculatePlaneBounds
    (
        ref Matrix4x3 world
    )
    {
    ...
    private unsafe void FillFromFileNode
    (
        List<MeshPart>    parts,
    ...
    private unsafe MeshPart BuildMeshFromNode
    (
        MeshPCB.FileNode* node
    )
    {
    ...
    private PrimitiveFlags ExtractMaterialFlags
    (
        ulong mat
    )
    {
    ...
    private static MeshPart FinalizePart
    (
        MeshPart part
    )
    {
    ...
    private static List<MeshPart> BuildSphereMesh
    (
        int numSegments
    )
    {
        ...
        for (var p = -maxParallel; p <= maxParallel; ++p)
        {
    ...
    private static List<MeshPart> BuildCylinderMesh
    (
        int numSegments
    )
    {
    ...
    private static List<MeshPart> BuildRampMesh()
    {
        var mesh = new MeshPart();
        mesh.Vertices.Add(new(-1, -1, -1));
        mesh.Vertices.Add(new(-1, -1, +1));
        ...
        mesh.Primitives.Add(new(0, 1, 4, PrimitiveFlags.None));
        mesh.Primitives.Add(new(4, 1, 5, PrimitiveFlags.None));
        mesh.Primitives.Add(new(2, 4, 3, PrimitiveFlags.None));
