...
public class NavmeshCustomization
{
    ...
    public virtual NavmeshSettings GetBuildSettings
    (
    ...
    public virtual void CustomizeScene
    (
    ...
    public virtual void CustomizeBuildProfile
    (
        SceneDefinition     definition,
    ...
    public virtual void CustomizeBuildSettings
    (
        SceneDefinition definition,
    ...
    public virtual void CustomizeMesh
    (
        Navmesh    mesh,
    ...
    protected internal virtual void ApplyBuildSettings
    (
        SceneDefinition definition,
        NavmeshSettings settings
    )
    ...
    private void ApplyLegacySettingsOverrides
    (
        NavmeshSettings settings
    )
    {
    ...
    )
    {
        ...
        LinkResolvedPoints
            (meshData, startRef, startTile, startPoly, endRef, endTile, endPoly, projectedStart, projectedEnd, area, flags, kind, bidirectional, traversalProfile);
    }
    ...
    )
    {
        ...
        (
            offMeshRef,
            new(projectedStart.ToSystem(), projectedEnd.ToSystem(), kind, bidirectional, 0, traversalProfile),
            true
        );
        ...
        if (bidirectional)
        {
            var reverseSide = offMeshConnection.side == 0xff ?
                                  (byte)0xff :
                                  (byte)DtUtils.OppositeTile(offMeshConnection.side);
    ...
    )
    {
        ...
        var status = query.FindNearestPoly(pos.ToRecast(), new(5, 5, 5), new DtQueryDefaultFilter(), out var polyRef, out var projectedPoint, out _);
