...
using NavmeshBuilder = vnavmesh.Common.Build.NavmeshBuilder;
...
public sealed class NavmeshManager : IDisposable
{
    ...
    public (Vector3 Min, Vector3 Max) BuildBitmap
    (
    ...
    private async Task<BuildNavmeshResult> BuildNavmesh
    (
    ...
    )
    {
        ...
        await using (var stream = new FileStream
                         (rawFile.FullName, FileMode.Open, FileAccess.Read, FileShare.Read, 1 << 22, FileOptions.SequentialScan | FileOptions.Asynchronous))
    ...
    )
    {
        ...
        await using (var sceneStream = new FileStream
                         (sceneFile.FullName, FileMode.Create, FileAccess.Write, FileShare.None, 1 << 22, FileOptions.SequentialScan | FileOptions.Asynchronous))
        await using (var sceneWriter = new BinaryWriter(sceneStream, Encoding.UTF8, true))
        {
    ...
    private static async Task<NavmeshBuildMessage> ReadBuildMessage
    (
        Stream            stream,
        CancellationToken cancel
    )
    ...
    private static async Task ReadExactlyOrThrow
    (
        Stream            stream,
        byte[]            buffer,
        CancellationToken cancel
    ...
    private static void LogWorkerOutput
    (
        string       streamName,
    ...
    internal static unsafe string GetCacheKey
    (
        SceneDefinition scene
    )
    {
    ...
    private static void MoveRawBuildToCache
    (
        string   cacheKey,
        FileInfo rawFile,
        FileInfo cache
    ...
    private Task<T> ExecuteWhenIdle<T>
    (
    ...
    // 等待前序任务完成, 同时响应取消令牌
    private static async Task WaitForPrevious
    (
        Task              prev,
        CancellationToken token
    )
    ...
    private static void LogCacheSegment
    (
        string                        action,
        Navmesh.CacheSegmentTelemetry segment
    ) =>
        Log
        (
    ...
    private static double FormatMiB
    (
        long bytes
    ) => bytes / 1024.0 / 1024.0;
    ...
    private void ReleaseRetiredState
    (
        Navmesh?      navmesh,
        NavmeshQuery? query,
        string        reason
    ...
    private static void FinalizeRetiredNavmeshRelease
    (
        Navmesh navmesh,
        string  reason
    )
    ...
    private static void RequestMemoryCompaction
    (
        string reason
    ) =>
        _ = Task.Run
    ...
    private static HashSet<long> CollectPruneSeedPolys
    (
    ...
    private static HashSet<long> FindPruneSeedPolys
    (
        DtNavMeshQuery query,
        Vector3        point
    )
    {
        ...
        {
            if (!query.ClosestPointOnPoly(poly, point.ToRecast(), out var closest, out _).Succeeded())
                continue;

            var closestPoint = closest.ToSystem();
    ...
    private static List<long> FindIntersectingMeshPolys
    (
        DtNavMeshQuery query,
        Vector3        point,
        Vector3        halfExtent
    )
    {
        ...
        while (true)
        {
            ...
            var status    = query.QueryPolygons(point.ToRecast(), halfExtent.ToRecast(), SPruneFilter, collector);
            if (status.Failed())
                return [];
    ...
    private static List<long> CollectTraversableMeshPolyRefs
    (
        DtNavMesh mesh
    )
    {
    ...
    private static int CountComponentPolys
    (
        IReadOnlyList<PruneComponent> components,
    ...
    private static string FormatHexNumbers<T>
    (
        IEnumerable<T> nums
    ) where T : INumber<T> =>
