...
public record class Navmesh(int CustomizationVersion, DtNavMesh Mesh, VoxelMap? Volume)
{
    ...
    private static DtMeshData DeserializeMeshTile(BinaryReader reader)
    {
        var tile = new DtMeshData
        {
            header = new()
            {
                magic          = DtNavMesh.DT_NAVMESH_MAGIC,
                version        = DtNavMesh.DT_NAVMESH_VERSION,
                x              = reader.ReadInt32(),
                y              = reader.ReadInt32(),
        ...
        for (var i = 0; i < tile.detailVerts.Length; ++i)
        ...
        for (var i = 0; i < tile.detailTris.Length; ++i)
    ...
    private static void SerializeMeshTile(BinaryWriter writer, DtMeshData tile)
    {
        ...
        for (var i = 0; i < tile.header.vertCount * 3; ++i)
        ...
        for (var i = 0; i < tile.header.detailVertCount * 3; ++i)
            writer.Write(tile.detailVerts[i]);

        writer.Write(tile.header.detailTriCount);
        for (var i = 0; i < tile.header.detailTriCount * 4; ++i)
