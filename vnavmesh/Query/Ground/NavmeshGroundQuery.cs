...
)
{
    ...
    )
    {
        ...
        var finalDestination = straightPath[straightPathCount - 1].pos.ToSystem();
    ...
    {
        if (query.MeshQuery.ClosestPointOnPoly(poly, point.ToRecast(), out var closest, out isOverPoly).Succeeded())
        {
    ...
    private PlannerResult LogMeshFailure
    (
        Vector3 from,
        Vector3 to,
        long    startRef,
    ...
    private static PathfindStatus ResolveStatus
    (
        long    lastPoly,
        long    endRef,
    ...
    ) : IDtQueryFilter
    {
        ...
        )
        {
            ...
            if (area == NavmeshArea.Ground)
                return Vector3.Distance(pa.ToSystem(), pb.ToSystem());
            ...
            var traversalProfile = navmeshData.TryGetOffMeshLink(curRef, out var link) ?
                                       ...
                                       null;
            return NavmeshLinkTraversalProfiles.EstimateCost(pa.ToSystem(), pb.ToSystem(), kind.Value, traversalProfile);
        }
        ...
        (
            long                             polyRef,
            out NavmeshLinkTraversalProfile? traversalProfile
        )
        {
