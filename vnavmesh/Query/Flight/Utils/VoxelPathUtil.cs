...
{
    public static int DetermineEffectiveLookBackDepth
    (
    ...
    public static List<(ulong voxel, Vector3 p)> MergePathSegments
    (
        List<(ulong voxel, Vector3 p)> head,
        List<(ulong voxel, Vector3 p)> tail,
        float                          epsilon
    )
    {
        if (head.Count == 0)
    ...
    public static void AppendPathPoint
    (
        List<(ulong voxel, Vector3 p)> output,
        (ulong voxel, Vector3 p)       point,
        float                          epsilon
    )
    {
    ...
    public static int FindPathPointIndex
    (
        IReadOnlyList<(ulong voxel, Vector3 p)> path,
        (ulong voxel, Vector3 p)                point,
        int                                     startIndex,
        float                                   epsilon
    )
