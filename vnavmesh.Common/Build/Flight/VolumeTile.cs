...
)
{
    ...
    public (int x, int y, int z) WorldToVoxel
    (
        Vector3 v
    )
    {
    ...
    public Vector3 VoxelToWorld
    (
        int x,
        int y,
        int z
    ) => BoundsMin + (new Vector3(x + 0.5f, y + 0.5f, z + 0.5f) * LevelDesc.CellSize);

    public Vector3 VoxelToWorld
    (
        (int x, int y, int z) v
    ) => VoxelToWorld(v.x, v.y, v.z);

    public (Vector3 min, Vector3 max) CalculateSubdivisionBounds
    (
        int x,
        int y,
        int z
    ...
    (
        (int x, int y, int z) v
    ) => CalculateSubdivisionBounds(v.x, v.y, v.z);
    ...
    public (ulong index, bool empty) FindLeafVoxel
    (
        Vector3 p,
    ...
    public IEnumerable<(ulong index, bool empty)> EnumerateLeafVoxels
    (
        Vector3 bmin,
        Vector3 bmax
    )
    ...
    public void EnsureSubdivisionCapacity
    (
        int capacity
    )
    {
    ...
    (
        IEnumerable<VolumeTile> children
    )
    {
    ...
    public VolumeTile GetSubdivision
    (
        int index
    )
    {
    ...
    private ushort GetPackedCellValue
    (
        int index
    ) => GetPackedCellState(index) switch
    {
    ...
    private VolumePackedCellState GetPackedCellState
    (
        int index
    )
