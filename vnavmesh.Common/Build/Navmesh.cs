...
public record class Navmesh
{
    ...
    (
        BinaryReader reader,
        int          expectedCustomizationVersion,
        string       expectedBuildSignature
    )
    {
        EnsureLittleEndian();
    ...
    public CacheTelemetry Serialize
    (
        BinaryWriter writer
    )
    {
    ...
    private static EncodedSegment EncodeSegment
    (
        CacheSegmentKind     kind,
        CacheCodec           codec,
        Action<BinaryWriter> serialize
    )
    {
    ...
    private static byte[] ReadSegmentPayload
    (
        Stream                 source,
    ...
    private static (T Value, CacheSegmentTelemetry Telemetry) DecodeSegment<T>
    (
        CacheSegmentDescriptor descriptor,
        byte[]                 payload,
    ...
    private static Stream OpenDecodedSegmentStream
    (
        Stream                 source,
        CacheSegmentDescriptor descriptor
    ) => descriptor.Codec switch
    {
    ...
    private static (byte[] Payload, int Offset, int Length) EncodePayload
    (
        byte[]     rawPayload,
        int        rawPayloadOffset,
        int        rawPayloadBytes,
        CacheCodec codec
    ) => codec switch
    {
    ...
    private static (byte[] Payload, int Offset, int Length) CompressFastLz
    (
        byte[] rawPayload,
        int    rawPayloadOffset,
        int    rawPayloadBytes
    ...
    private static byte[] DecompressFastLz
    (
        byte[] payload,
        long   expectedBytes
    )
    ...
    private static CacheSegmentDescriptor ReadSegmentDescriptor
    (
        BinaryReader reader
    ) => new
    (
    ...
    private static void WriteSegmentDescriptor
    (
        BinaryWriter           writer,
        CacheSegmentDescriptor descriptor
    )
    ...
    private static float[] ReadSingleArray
    (
        BinaryReader reader,
        int          count
    )
    ...
    private static void WriteSingleArray
    (
        BinaryWriter        writer,
    ...
    private static int[] ReadByteBackedIntArray
    (
        BinaryReader reader,
        int          count
    )
    ...
    private static void WriteByteBackedIntArray
    (
        BinaryWriter      writer,
    ...
    private static (Vector3 min, Vector3 max) DeserializeBounds
    (
        BinaryReader reader
    ) => (DeserializeVector3(reader), DeserializeVector3(reader));

    private static void SerializeBounds
    (
        BinaryWriter writer,
        Vector3      min,
        Vector3      max
    ...
    private static void SerializeVector3
    (
        BinaryWriter writer,
        Vector3      v
    )
    ...
    private static DtNavMesh DeserializeMesh
    (
        BinaryReader reader
    ) =>
        new DtMeshSetReader().Read(reader);

    private static (byte[] Payload, CacheSegmentTelemetry Telemetry) DecodeDeferredMeshSegment
    (
        CacheSegmentDescriptor descriptor,
        Stream                 source
    )
    ...
    private static DtNavMesh DeserializeMeshPayload
    (
        byte[]     payload,
        CacheCodec codec,
        long       expectedBytes
    ...
    private static void SerializeMesh
    (
        BinaryWriter writer,
        DtNavMesh    mesh
    ) =>
    ...
    private static CacheSegmentTelemetry CombineVolumeTelemetry
    (
        CacheSegmentTelemetry header,
        CacheSegmentTelemetry tree
    ) => new
    (
        CacheSegmentKind.Volume,
    ...
    private void SetDeferredMeshPayload
    (
        byte[]     payload,
        CacheCodec codec,
        long       expectedBytes
    ...
    public void DeferMeshMutation
    (
        Action<DtNavMesh> mutator
    ) =>
        _deferredMeshMutator += mutator;

    public void RegisterOffMeshLink
    (
        long        polyRef,
        NavmeshLink link,
        bool        includeInVisualization = false
    )
    {
    ...
    public bool TryGetOffMeshLink
    (
        long            polyRef,
        out NavmeshLink link
    ) =>
